...
Checks:
  ...
  5. P&L drawdown within acceptable limits (INTRADAY only, resets daily)

Alerts via Discord on ANY failure, with cooldown to prevent spam.
Self-healing: attempts to restart gateway if authentication fails.

Run: cron entry — */5 13-20 * * 1-5 (every 5 min during market hours)
"""
...
ALERT_COOLDOWN  = 1800    # 30 minutes between repeated drawdown alerts
...
def load_state() -> dict:
    if STATE_FILE.exists():
        try:
            ...
            pass
    return {"last_nav": 0.0, "peak_nav": 0.0, "peak_date": "",
            "consecutive_failures": 0, "last_alert": None,
            "last_dd_alert": None, "positions": {}}
...
async def run_check():
    ...
    if not api_ok:
        ...
        if state["consecutive_failures"] >= 3:
            ...
            discord("API down for 3+ checks. Attempting gateway restart...", urgent=True)
            restart_gateway()
        ...
        # ── Reset peak_nav daily (track INTRADAY drawdown, not all-time) ──
        if state.get("peak_date", "") != today:
            state["peak_nav"] = nav
            state["peak_date"] = today
            state["last_dd_alert"] = None
            log.info("New trading day — reset peak NAV to $%.2f", nav)

        state["peak_nav"] = max(state.get("peak_nav", nav), nav)

        # ── Check 3: Intraday NAV drawdown ──
        intraday_dd = (nav - state["peak_nav"]) / max(state["peak_nav"], 1)
        if intraday_dd < MAX_DD_ALERT:
            # Only alert if cooldown has passed
            last_dd = state.get("last_dd_alert")
            should_alert = True
            if last_dd:
                try:
                    elapsed = (datetime.now() - datetime.fromisoformat(last_dd)).total_seconds()
                    if elapsed < ALERT_COOLDOWN:
                        should_alert = False
                        log.warning("DRAWDOWN %.1f%% (alert suppressed, cooldown %ds remaining)",
                                    intraday_dd * 100, ALERT_COOLDOWN - elapsed)
                except Exception:
                    pass

            if should_alert:
                issues.append(f"Intraday drawdown: {intraday_dd*100:.1f}% (${state['peak_nav']:.2f}→${nav:.2f})")
                state["last_dd_alert"] = datetime.now().isoformat()
                log.warning("DRAWDOWN ALERT: %.1f%%", intraday_dd * 100)
            # If suppressed, don't add to issues list → no Discord spam
        ...
        if prev_positions and prev_positions != new_positions:
            ...
            if added or removed:
                log.info("Position change: +%s -%s", added, removed)
    ...
    # ── Send issue alerts (only truly new ones) ──
    if issues:
