...
class CircuitBreakerConfig:
    ...
    max_drawdown_pct: float = -0.15   # C-03 fix: halt at -15% drawdown from peak
    max_daily_loss_pct: float = -0.05  # halt at -5% daily loss
    max_consecutive_losses: int = 5
    max_open_positions: int = 8        # C-05 fix: align with risk_box limit
    max_orders_per_minute: int = 20
    cooldown_minutes: float = 120.0    # C-05 fix: 2-hour cooldown, not 7 days
...
class KillSwitch:
    ...
    def __init__(
        ...
        initial_equity: float = 6000.0,  # C-04 fix: default to actual account size
    ) -> None:
